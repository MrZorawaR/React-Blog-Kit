...
export default function BlogPostPage() {
  ...
  const { data: relatedPosts = [] } = useQuery({
    ...
    queryFn: () => getRelatedBlogs(blog!.id, 4),
    enabled: !!blog?.id
  })

  if (isLoading) {
    return (
      <div className="min-h-screen bg-[#F8F9FC] py-10 flex items-center justify-center">
    ...
    return (
      <div className="min-h-screen bg-[#F8F9FC] py-10 flex items-center justify-center">
        <div className="text-center">
          ...
          <Link to="/" className="text-blue-600 hover:underline">← Back to blogs</Link>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FC] py-10">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 mb-8">
        <Link to="/" className="inline-flex items-center text-sm font-bold text-blue-600 hover:underline gap-1">
