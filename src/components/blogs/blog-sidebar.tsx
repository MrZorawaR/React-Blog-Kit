...
export function RelatedArticles({ articles }: { articles: { id: string; title: string; slug: string; image_url: string | null }[] }) {
  return (
    <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
      <h3 className="font-bold text-gray-900 mb-6">Related Articles</h3>
      <div className="space-y-6">
        {articles.map((post) => (
          <Link key={post.id} to={`/blogs/${post.slug}`} className="flex gap-4 group">
